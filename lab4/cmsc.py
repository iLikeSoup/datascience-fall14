...
for i in range(length):
    ...
    my_ind = [str(x) for x in range(numRows)]

    df_courseNum = pd.DataFrame({'Course No.': courseNum[i]}, index=my_ind)
    df_sectionNum = pd.DataFrame({'Section No.': sectionNum}, index=my_ind)
    df_instructor = pd.DataFrame({'Instructor': instructor}, index=my_ind)
    df_seats = pd.DataFrame({'Seats': [x[0] for x in seats_open_waitlist]}, index=my_ind)
    df_open = pd.DataFrame({'Open': [x[1] for x in seats_open_waitlist]}, index=my_ind)
    df_waitlist = pd.DataFrame({'Waitlist': [x[2] for x in seats_open_waitlist]}, index=my_ind)
    df_days = pd.DataFrame({'Days': [x[0] for x in days_time]}, index=my_ind)
    df_time = pd.DataFrame({'Time': [x[1] for x in days_time]}, index=my_ind)
    df_building = pd.DataFrame({'Building': [x[0] for x in building_room]}, index=my_ind)
    df_room = pd.DataFrame({'Room': [x[1] for x in building_room]}, index=my_ind)
