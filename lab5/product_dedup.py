...
def customComparator(field_1, field_2) :
	...
	f1 = re.findall('([\d]+(?:[.][\d]+)?)(?: gbp)?', field_1)
	f2 = re.findall('([\d]+(?:[.][\d]+)?)(?: gbp)?', field_2)
	#print('field_1: {}\nf1: {}'.format(field_1, f1))
	#print('field_2: {}\nf2: {}'.format(field_2, f2))
	if ''.join(f1) == ''.join(f2):
		return 1
	else:
		return 0
    else :
	#print 'returning nan for {} and {}'.format(field_1, field_2)
        return nan
...
def customComparator2(field_1, field_2):
	if field_1 and field_2:
		...
		#print('returning nan in cc2(). f1: {}, f2: {}'.format(field_1, field_2))
		return nan
...
if os.path.exists(settings_file):
    ...
    fields = [
	{'field' : 'title', 'type': 'String', 'comparator' : customComparator2},
	{'field' : 'manufacturer', 'type': 'String', 'has missing':True, 'comparator' : customComparator2},
        {'field' : 'price', 'type': 'Custom', 'has missing':True, 'comparator' : customComparator}
        ]

    # Create a new deduper object and pass our data model to it.
    print 'deduper data model passing'
